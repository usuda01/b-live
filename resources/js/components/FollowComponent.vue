<template>
    <div class="followers">
        <div v-for="follower in followers" :key="follower.id" class="follower-box">
            <a class="user-profile" v-bind:href="'/user/' + follower.follow_id" v-bind:style="{ backgroundImage: 'url(' + follower.user_image_path + ')' }"></a>
            <div class="user-right">
                <div class="user-name"><a v-bind:href="'/user/' + follower.follow_id">{{ follower.follow_user.name }}</a></div>
    ...
    export default {
        ...
        methods: {
            ...
            infiniteHandler($state) {
                axios.get('/api/followers/follows', {
                    params: {
                        page: this.page,
                        per_page: 1,
                    },
