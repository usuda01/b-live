    ...
    <div class="room-stream" :class="{'webview': isApp === true, 'rotate': isRotate === true, 'controller-active': isControllerClassActive === true}">
        ...
        <div class="supporter" v-bind:class="{'active': activeTab === 3}">
            <div v-for="supporter in supporters" :key="supporter.id">
                ...
                <div v-else-if="supporter.product_id === '7'" class="user-message payment purchase7"><a v-on:click.prevent="showUserInfo(supporter.user)" href="#" class="user-profile" v-bind:style="{ backgroundImage: 'url(' + supporter.user.image_path + ')' }"></a><a v-on:click.prevent="showUserInfo(supporter.user)" href="#" class="user-name">{{ supporter.user.name }}</a><span class="amount">&yen;{{ supporter.point }}</span><span class="message">{{ supporter.message.content }}</span></div>
            </div>
        </div><!--// .supporter -->
